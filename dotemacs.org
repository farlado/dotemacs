...
This file is an attempt at as close to a literate Emacs configuration as possible. Admittedly, much of the "documentation" in here is more commentary I provide about my choices and personal feelings about certain things in Emacs based on my admittedly very limited opinion of the program. Regardless, the idea is that this file, when tangled, generates everything in my repository, most importantly my ~init.el~. I would /highly/ recommend you *NOT* try to read the ~init.el~ file itself. It's a jungle, especially since all of the commentary regarding the configuration is in this file and not in ~init.el~.
** Why a "literate" ~init.el~?
The answer is rather simple. Before I used a literate-like file for my entire configuration, I had an ~init.el~ which tangled blocks from a ~config.org~ on startup. It turns out this was abysmally slow (around 4.5 seconds on a cold startup), and that tangling blocks from a ~dotemacs.org~ into an ~init.el~ would cut that time by about a third (around 2.8 seconds). This also comes with the advantage of being a hub for /all/ things related to my Emacs configuration, where previously I had to check both ~init.el~ and ~config.org~ when I thought I might have changed something for the worse.
** Installation
...
- For Haskell: ~stack~
** License
Because I +am a lazy piece of shit who doesn't like long licenses+ do not think much harm could come from using a pushover license for this specific configuration, I am publishing this literate Emacs configuration under the WTFPL.
...
Since I do not use ~customize~ but really like protecting packages used in my config from ~package-autoremove~, I need to still set ~package-selected-packages~ so that it'll work.
#+BEGIN_SRC emacs-lisp
...
*** Tangle my personal config when it's saved
This file is used to produce my ~init.el~, so it would be pretty dumb for it not to tangle when I'm configuring it.
#+BEGIN_SRC emacs-lisp
