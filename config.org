...
** Terminal
I've been jumping between ~vterm~ and ~ansi-term~ but ~ansi-term~ seems to support more of the fancy stuff I like about Emacs. This ensures ~ansi-term~ always starts with my default shell.
#+BEGIN_SRC emacs-lisp
  (defvar my-term-shell "/bin/zsh"
    "The shell to start `term' and `ansi-term' with.")
...
#+END_SRC
Then, we need to determine what size to make the text. This is based on the width of the screens I regularly use.
#+BEGIN_SRC emacs-lisp
  (let* ((monitor-count (if (string= (window-system) 'x)
			    (string-to-number
			     (shell-command-to-string
			      "xrandr | grep \\* | wc -l"))
			  1))
	 (res (/ (display-pixel-width) monitor-count))
	 (size (if (< res 1920)
		   100
		 (if (< res 2880)
		     150
