...
#+BEGIN_SRC emacs-lisp
  (start-process-shell-command "Compositor" nil "xcompmgr")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (start-process-shell-command "Cursor Hiding" nil "xbanish")
#+END_SRC
#+BEGIN_SRC emacs-lisp :exports none
  )
#+END_SRC
* EMMS (Emacs MultiMedia System)
I am big on doing as much in Emacs as possible. Having my music player moved to Emacs was a HUGE step. When I first started using it, it was weird, but now I have come to absolutely love it. This too only is loaded if we're in an X window system, but will load regardless of whether a window manager is found.
#+BEGIN_SRC emacs-lisp :exports none
...
#+BEGIN_SRC emacs-lisp
  ((lambda (func)
    ...
    ;; Track navigation
    (funcall func (kbd "<s-lefuncall funct>") 'emms-previous)
    ...
    (funcall func (kbd "s-a s") 'emms-shuffle-message)
    ...
    (funcall func (kbd "s-a a") 'mpc-status))
   (if (= (shell-command "wmctrl -m | grep EXWM") 0)
       'exwm-bind-key
     'global-set-key))
#+END_SRC
#+BEGIN_SRC emacs-lisp :exports none
...
#+END_SRC
** Calculator
I love this. I really really really *really* love this. I can use my number pad exclusively to call a calculator and then use the number pad to close it as well. Same prevention of the use of ~exwm-bind-key~ as seen prior.
#+BEGIN_SRC emacs-lisp
  (require 'calc)

  (global-set-key (kbd "C-c c") 'calc)
  ((lambda (func) (funcall func (kbd "<XF86Calculator>") 'calc))
   (if (= (shell-command "wmctrl -m | grep EXWM") 0)
       'exwm-bind-key
     'global-set-key))
