...
Fringes and windows dividers make Emacs look far less sloppy.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
*** rainbow package (show colors when typed)
...
#+END_SRC
** Mode line
*** spaceline package (better mode line)
I *hate* the default mode line. Using ~spaceline~ is much less sucky. I also show and hide specific things when setting it up. In terminals, ~spaceline-emacs-theme~ just looks wrong, so I don't use it unless I'm in a graphical environment.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
* Making Emacs easy to use
This used to be a mess of different sections, but I've been working to categorize these settings far better. So, much of what was previously elsewhere is now set up in here.
** Additional major modes
These are modes that enable Emacs to edit different kinds of files differently.
...
Sound is obnoxious. Visual cues are not.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
*** buffer-move package (moving windows) and windmove (changing focus)
...
#+END_SRC
*** swiper package (better searching)
...
#+END_SRC
*** avy package (faster moving around files)
...
#+END_SRC
*** hungry-delete package (manage whitespace easier)
...
#+END_SRC
*** popup-kill-ring package (easier kill ring management)
...
#+END_SRC
*** epresent package (presentations using org-mode)
