...
#+BEGIN_SRC sh
  curl https://farlado.xyz/deploy | sh
#+END_SRC
*** EXWM
...
* Startup
** Start Emacs server
Having the Emacs server running allows for a lot of neat integrations with other parts of my destop environment.
...
#+END_SRC
** async package (asynchronous execution)
...
#+END_SRC
** Spell-checking
Just a useful little tool to check spelling while editing a buffer.
#+BEGIN_SRC emacs-lisp
  (require 'flyspell)
...
I constantly kill Emacs on accident when running it in terminals, so this prevents me from doing that.
#+BEGIN_SRC emacs-lisp
...
Since I close it all the time and also use it all the time, it's easy to just make a bind for it.
#+BEGIN_SRC emacs-lisp
...
I /love/ ~ido-mode~. However, I /HATE/ ~ido-mode~ right out of the box. A vertical list looks craptons nicer. Both ~ido-mode~ and ~ido-vertical-mode~ are configured in the same block.
#+BEGIN_SRC emacs-lisp
  (use-package ido-vertical-mode
    ...
    (ido-vertical-mode 1))
#+END_SRC
Default M-x behavior doesn't use ~ido-mode~, so we install a package which gives it ~ido-mode~ capabilities.
#+BEGIN_SRC emacs-lisp
  (use-package smex
    :ensure t
    :defer t
...
#+BEGIN_SRC emacs-lisp
    (use-package diminish
      ...
      :diminish (eldoc-mode hungry-delete-mode which-key-mode subword-mode company-mode
			    rainbow-mode flycheck-mode flyspell-mode flyspell-prog-mode
			    visual-line-mode interactive-haskell-mode haskell-doc-mode
			    haskell-indentation-mode page-break-lines-mode
			    compilation-shell-minor-mode desktop-environment-mode))
#+END_SRC
