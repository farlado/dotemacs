...
#+END_SRC
*** Volume adjustment
The only things I really don't like here are how much output it gives when you mute or unmute the speakers or microphone, so I set up a basic script to give much more concise output.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
*** Screenshots
This one was the least straightforward. First, I just use the current directory for this.
#+BEGIN_SRC emacs-lisp
  (setq desktop-environment-screenshot-directory ".")
#+END_SRC
Then, I can set the command for a full-screen screenshot. The way the bind is supposed to be used is to store a screenshot in the directory we just defined, but I would rather put it in the clipboard.
#+BEGIN_SRC emacs-lisp
  (setq desktop-environment-screenshot-command
	"maim /dev/stdout | xclip -selection -clipboard -t image/png")
#+END_SRC
Lastly, I can set the command for a partial screenshot. Again, it gets stored in the clipboard.
#+BEGIN_SRC emacs-lisp
  (setq desktop-environment-screenshot-partial-command
	"maim -s /dev/stdout | xclip -selection -clipboard -t image/png")
#+END_SRC
*** Lockscreen
I use a personal script for this one, see [[https://github.com/farlado/dotfiles][my dotfiles]] for .local/bin/i3l to understand.
#+BEGIN_SRC emacs-lisp
  (setq desktop-environment-screenlock-command "i3l")
#+END_SRC
** Network Settings
