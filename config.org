...
#+END_SRC
**** Discord
Yeah, I also use a light theme for Discord. It looks comfy and consistent with the rest of my system, even if Discord is a garbage application.
#+BEGIN_SRC emacs-lisp
  (defun run-discord ()
    "Start Discord."
    (interactive)
...
Binding keys using ~use-package~ is far easier than defining a keymap.
#+BEGIN_SRC emacs-lisp
  (use-package buffer-move
    :ensure t
    :defer t
    :bind (("C-x C-o w" . buf-move-up)
	   ("C-x C-o a" . buf-move-left)
	   ("C-x C-o s" . buf-move-down)
	   ("C-x C-o d" . buf-move-right)))
#+END_SRC
* Org-mode
