  ...
  (defun cycle-kbd-layout ()
    ...
    (let* ((current-layout (shell-command-to-string
			    (concat "setxkbmap -query | "
				    ...
				    "tr '\n' ' ' | sed 's/ //'")))
	   (new-layout (if (string= current-layout "us")
			   "epo"
			 (if (string= current-layout "epo")
			     "de"
			   "us"))))
      (shell-command (concat "setxkbmap " new-layout " -option ctrl:nocaps"))
      (message "Keyboard layout is now: %s" new-layout)))

  (defun cycle-kbd-layout-reverse ()
    "Cycle between keyboard layouts in reverse."
    (interactive)
    (let* ((current-layout (shell-command-to-string
			    (concat "setxkbmap -query | "
				    "grep -oP 'layout:\\s*\\K(\\w+)' | "
