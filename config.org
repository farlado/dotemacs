...
#+END_SRC
** Set the keyboard layout to US and give caps lock the functionality of control
...
#+END_SRC
*** Volume adjustment
The only things I really don't like here are how much output it gives when you mute or un-mute the speakers or microphone, so I set up basic scripts to give much more concise output.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-n") 'network-settings)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun cycle-keyboard-layout ()
    ...
    (interactive)
    (let* ((current-layout (shell-command-to-string
  ...
  (defun cycle-keyboard-layout-reverse ()
	...
	(message "No keyboard layouts selected."))))
#+END_SRC
Lastly, I set my preferred keyboard layouts and bind the functions to keys.
#+BEGIN_SRC emacs-lisp
  (setq keyboard-layout-1 "us"
	keyboard-layout-2 "epo"
	keyboard-layout-3 "de")

  (exwm-bind-key (kbd "s-SPC") 'cycle-keyboard-layout)

  (exwm-bind-key (kbd "<s-backspace>") 'cycle-keyboard-layout-reverse)
#+END_SRC
** Suspending
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package emms
    ...
    (exwm-bind-key (kbd "s-a v") 'emms)
    (exwm-bind-key (kbd "s-a b") 'emms-smart-browse)
    (exwm-bind-key (kbd "s-a r c") 'emms-player-mpd-update-all-reset-cache)
    (exwm-bind-key (kbd "<s-left>") 'emms-previous)
    (exwm-bind-key (kbd "<s-right>") 'emms-next)
    (exwm-bind-key (kbd "<s-up>") 'emms-stop)
    (exwm-bind-key (kbd "<s-down>") 'emms-pause)
    (exwm-bind-key (kbd "<XF86AudioPrev>") 'emms-previous)
    (exwm-bind-key (kbd "<XF86AudioNext>") 'emms-next)
    (exwm-bind-key (kbd "<XF86AudioStop>") 'emms-stop)
    (exwm-bind-key (kbd "<XF86AudioPlay>") 'emms-pause))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-a x") 'mpd/start-music-daemon)
#+END_SRC
*** Stopping the daemon
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-a q") 'mpd/kill-music-daemon)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-a r d") 'mpd/update-database)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-a a") 'mpc-status)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-a s") 'emms-shuffle-message)
#+END_SRC
*** Setting repeat mode
#+BEGIN_SRC emacs-lisp
  (exwm-bind-key (kbd "s-a r a") 'emms-toggle-repeat-playlist)
  (exwm-bind-key (kbd "s-a r t") 'emms-toggle-repeat-track)
#+END_SRC
* Other programs
...
#+BEGIN_SRC emacs-lisp
  (use-package vterm
    ...
    :defer t
    :init
    (exwm-bind-key (kbd "<s-return>") 'vterm))
#+END_SRC
*** Calculator
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "<XF86Calculator>") 'calc)
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-f") 'run-firefox)
#+END_SRC
*** LibreOffice
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-b") 'run-libreoffice)
#+END_SRC
*** GIMP
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-g") 'run-gimp)
#+END_SRC
*** Telegram
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-d") 'run-discord)
#+END_SRC
*** Steam
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-bind-key (kbd "s-s") 'run-steam)
#+END_SRC
...
#+END_SRC
** graphviz-dot-mode package (diagrams creation)
...
Previously I used ~switch-window~ but it's not very intuitive, so here's something that works a little more intuitively for me. I also remove C-x o as a bind, since I don't like ~other-window~ either.
#+BEGIN_SRC emacs-lisp
  (exwm-bind-key (kbd "C-s-w") 'windmove-up)
  (exwm-bind-key (kbd "C-s-a") 'windmove-left)
  (exwm-bind-key (kbd "C-s-s") 'windmove-down)
  (exwm-bind-key (kbd "C-s-d") 'windmove-right)
...
#+BEGIN_SRC emacs-lisp
  (use-package buffer-move
    ...
    :defer t
    :init
    (exwm-bind-key (kbd "<C-s-up>")    'buf-mode-up)
    (exwm-bind-key (kbd "<C-s-down>")  'buf-mode-down)
    (exwm-bind-key (kbd "<C-s-left>")  'buf-move-left)
    (exwm-bind-key (kbd "<C-s-right>") 'buf-move-right))
#+END_SRC
* Org-mode
