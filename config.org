...
#+BEGIN_SRC emacs-lisp
  (shell-command "xset s off -dpms 1> /dev/null 2> /dev/null")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (shell-command "setxkbmap us -option ctrl:nocaps 1> /dev/null 2> /dev/null")
#+END_SRC
*** Set fallback cursor
Some X windows will have weird cursors if this isn't done.
#+BEGIN_SRC emacs-lisp
  (shell-command "xsetroot -cursor_name left_ptr 1> /dev/null 2> /dev/null")
#+END_SRC
*** Compositor
