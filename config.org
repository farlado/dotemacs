...
This one is a pleasant surprise to have honestly. Having Emacs handle system packages as well as its own is a very pleasant surprise.
#+BEGIN_SRC emacs-lisp
  (use-package system-packages
    ...
    :bind (("C-c p i" . system-packages-install)
	   ...
	   ("C-c p n" . system-packages-get-info)))
#+END_SRC
...
#+END_SRC
*** Multi-head
It's an Elisp function now, woo! It's very spaghetti, but it /kinda/ works so it's not a big deal.
#+BEGIN_SRC emacs-lisp
  (defun display-and-dock-setup ()
    "Configure displays and some peripherals without the need for an external shell script."
    (let ((monitors (split-string (shell-command-to-string
				    "xrandr | grep ' connected ' | awk '{print $1}'")))
	   (command "xrandr "))
      ;; This section specifically prepares the xrandr command
	...
	(let* ((output (concat "--output " monitor " "))
	       ...
	       (pos (concat "--pos "
				...
				"0x0 "
				  ...
				  "1080x0 "
				"3000x0 ")))))
	  ;; Disable eDP-1-1 if we're docked (e.g. DP-1-2-1 is connected)
	  ;; Otherwise, add the monitor and its characteristics to the command string
	  (if (and (string= monitor "eDP-1-1")
		 (member "DP-1-2-1" monitors))
	      (setq command (concat command output "--off "))
	  (setq command (concat command output primary rate res pos)))))
      ;; Finally, run the command we just created as a process.
      (start-process-shell-command
       "Display Setup" nil command))
    ;; Now, we can configure some peripherals.
    (let ((trackball (if (= (shell-command "xinput list | grep ELECOM") 0)
			 (shell-command-to-string (concat "xinput list | "
							  "grep ELECOM | "
							  "awk '{print $8}' | "
							  "sed 's/id=//' | "
							  "tr '\n' ' '"))
		       nil))
	  (trackpad (if (= (shell-command "xinput list | grep Synaptics") 0)
			(shell-command-to-string (concat "xinput list | "
							 "grep Synaptics | "
							 "awk '{print $6}' | "
							 "sed 's/id=//' | "
		      ...
		      nil)))
      (when trackball
	(dolist (cmd '((concat "xinput set-prop " trackball
			       "'libinput Button Scrolling Button' 10")
		       (concat "xinput set-prop " trackball
			       "'libinput Scroll Method Enabled' 0 0 1")
		       (concat "xinput set-button-map " trackball
			       "1 2 3 4 5 6 7 8 9 2 1 2"))
		     (start-process-shell-command
		      "Trackball Setup" nil cmd))))
      (when trackpad
	(start-process-shell-command
	 "Disable Trackpad" nil (concat "xinput disable " trackpad)))
      (start-process-shell-command
       "Keyboard Setup" nil "setxkbmap -option ctrl:nocaps")))

  (display-and-dock-setup)
...
** Terminal
I've been jumping between ~vterm~ and ~ansi-term~. I guess ~ansi-term~ wins out because ~vterm~ does not actually provide much benefit in comparison. Regardless, I use an alias ~run-terminal~ so that I only have to change this single block, instead of other blocks as well.
#+BEGIN_SRC emacs-lisp
