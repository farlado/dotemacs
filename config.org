...
#+BEGIN_SRC emacs-lisp
  (defun display-and-dock-setup ()
    ...
    (let ((trackball (when (= (shell-command "xinput list | grep ELECOM > /dev/null") 0)
		       (shell-command-to-string
			(concat "xinput list | grep ELECOM | head -n 1 | "
				"sed -r 's/.*id=([0-9]+).*/\\1/' | tr '\n' ' '"))))
	  (trackpad (when (= (shell-command "xinput list | grep Synaptics > /dev/null") 0)
		      (shell-command-to-string
		       (concat "xinput list | grep Synaptics | head -n 1 | "
			       "sed -r 's/.*id=([0-9]+).*/\\1/' | tr '\n' ' '"))))
      (when trackball
	(start-process-shell-command
	 "Trackball Setup" nil (concat "sleep 2 && xinput set-prop " trackball
				     "'libinput Button Scrolling Button' 10"))
	(start-process-shell-command
	 "Trackball Setup" nil (concat "sleep 2 && xinput set-prop " trackball
				     "'libinput Scroll Method Enabled' 0 0 1"))
	(start-process-shell-command
	 "Trackball Setup" nil (concat "sleep 2 && xinput set-button-map "
				     trackball "1 2 3 4 5 6 7 8 9 2 1 2")))
      (when trackpad
	(start-process-shell-command
