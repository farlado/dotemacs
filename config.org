...
#+BEGIN_SRC emacs-lisp
  (defun audio-loopback ()
    ...
    (dolist (sink '("loop" "out"))
      ...
      (shell-command (concat "pacmd update-sink-proplist " sink " device.description=" sink)))
    ;; Loop `loop' to primary output, pipe it to `out', loop primary into to `out'
    (dolist (command '("source=loop.monitor"
...
I constantly accidentally kill Emacs when running it in terminals, so this prevents me from doing that.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key ibuffer-mode-map (kbd "Q") 'kill-this-buffer-and-window)
#+END_SRC
