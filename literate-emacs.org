...
*** Update packages
Because every package update means the need for a new dump image, it is paramount we ensure the newest versions are installed. Before we can even do that, we need to ensure the package that allows batch updating is installed. Only do all this if we're connected to the internet.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp :noweb-ref transparency
