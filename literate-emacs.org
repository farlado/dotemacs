...
*** Shortcuts for various snippets in ~org-mode~ 9.2 and above
#+begin_src emacs-lisp
  (unless (< (string-to-number org-version) 9.2)
#+end_src

There are multiple items to handle here:
1. Don't handle arrow brackets as enclosures in org-mode
#+begin_src emacs-lisp
...
#+end_src
2. Load ~org-tempo~, the extension that allows the old way of doing things.
#+begin_src emacs-lisp
  (require 'org-tempo)
#+end_src
3. Add block shortcuts
#+begin_src emacs-lisp
