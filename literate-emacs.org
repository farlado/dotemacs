...
** Okay, that's pretty neat. Why though?
   ...
   efficiency improvements, of course) cuts this time down to around 1.5 to 1.7
...
** Starting Emacs FAST
   ...
   around 2.5 seconds. Now I am down to between 1.5 and 1.7 seconds, having cut
   about half of the start time out.
...
** Mode line
   ...
   I hate the default mode line with a burning passion. Costing me an additional
   four tenths of a second at startup, the mode line from Spacemacs is much more
...
*** Show clock and battery level on mode line
    ...
    The clock should be set to 24-hour time.
...
*** Hide minor modes from the mode line
    ...
    The modeline is quite crowded when there's so many minor modes present. This
    package makes it a lot cozier by removing those minor modes. However, it
    seems as though it doesn't really work unless it's loaded after every other
    package to ensure all the modes to diminish are loaded.
