...
*** Setting the wallpaper(s) with ~feh~
This is where it all comes together. I have a [[https://github.com/farlado/wallpapers][wallpaper repository]] which I keep in =$XDG_CONFIG_HOME/wallpapers= from which random wallpapers are chosen for each monitor.
#+begin_src emacs-lisp
  (defun set-wallpaper ()
