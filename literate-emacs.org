...
* About this configuration
  ...
  Literate programming is a method of programming in which programming is worked
  on in parallel with the development of documentation specifically pertaining
  to how the program was written, alongside how to use the program at times. The
  end product is a file that can be /tangled/ into source code and /weaved/ into
  ...
  This configuration is /incredibly/ opinionated. Hopefully with recent changes
  this file will contain enough commentary for you to understand everything
  within it. Regardless, the idea is that this file, when tangled, generates
...
** License
   ...
   #+begin_src text :tangle (locate-user-emacs-file "LICENSE")
...
** Files to ignore
   ...
   #+begin_src text :tangle (locate-user-emacs-file ".gitignore")
...
*** Show clock and battery level on mode line
    ...
    I used to use ~fancy-battery~ for battery level but it constantly disappeared on my
    teeny tiny screens so I just decided not to bother with it. Plus it's one less
    package to configure lol. The clock should be set to 24-hour time.
...
*** Put ~ivy~ menus in special frames
    ...
    Okay, it's important to force position after all. This ensures that every
    time a posframe is moved, it always moves to the proper place and is the
    proper size.
