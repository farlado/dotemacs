...
*** =pdumper.el=
    ...
    #+begin_src emacs-lisp :noweb yes :tangle "lisp/pdumper.el"
...
*** Pre-load the theme
    ...
    the theme ahead of time saves a significant portion of the time cut by using
    a portable dump image.
...
** Early package management
   ...
   call of ~package-initialize~ between that =early-init.el= and =init.el=, which
...
*** Configure package repositories
    ...
    MELPA repositories should be enough to last me decades.
...
*** Show line/column numbers on the mode line
    ...
    the point is on should always be visible on the mode line.
...
*** Turn ^L into pretty lines
    ...
    time than never on.  I didn't know it was an external package until I
    uninstalled ~dashboard~ for a short time.
...
*** Line numbers (on most buffers)
    ...
    I don't like line numbers in modes where it breaks the mode.
...
*** Emacs is my terminal
    ...
    like the best path is to just stick to ~ansi-term~ with some of the more
...
* Making Emacs a desktop environment
  ...
  section if you don't plan to use Emacs as your desktop environment.  Including
  it doesn't have any disadvantages though, since it only loads if an
...
*** Getting a ~rofi~ equivalent
    ...
    Suckless project, but laziness makes that hard.  At some point I will make
    my own function, preferably one considerably more minimalist.
...
** Multi-head configuration
   ...
   displays and is used for different purposes.  Due to now not-so-recent
   updates to Arch Linux and the fact that the original code here was a dumpster
   fire, this section had to be reworked.
...
*** Keyboard layout selection
    ...
    little easier to manage... hopefully. Because of this, =C-\= needs to be added
...
*** =pdumper.el=
    ...
    #+begin_src emacs-lisp :tangle "lisp/pdumper.el"
      
