...
** Font style
   ...
   :header-args: :tangle (user-emacs-file "init.el")
   :end:
*** Setting the main font style
Originally I had this set up through ~customize~, but frankly that is far more difficult to configure than this method. Determine whether my preferred font is present on the system and set it if present. Otherwise, keep whatever default font Emacs chooses. We also set the font size here.
#+begin_src emacs-lisp
...
** Mode line
I've used many different mode lines, but personally I think the best one is not having a mode line to begin with, and instead having information in the echo area. This package does has some.... interesting side effects, but none that are too worrying and only really affect terminals.
...
*** Hide minor modes I don't care about
These just take up space and make the status area jitter like crazy. I know what modes I use, so I don't need to CONSTANTLY see them.
#+begin_src emacs-lisp :noweb-ref diminish-modes
...
#+begin_src emacs-lisp :noweb-ref lines-and-columns
  (line-number-mode 1)
  (column-number-mode 1)
#+end_src

** Leuven
Leuven is absolutely unbeatable. There is very little that I actually have to change about it, and none of them is a fundamental failing of Leuven itself. It's nearly perfection.
#+begin_src emacs-lisp :noweb yes :tangle (user-emacs-file "init.el")
...
*** Mode line
For some reason, the mode line uses /white/ for one of its colors. Screw that.
#+begin_src emacs-lisp :noweb-ref mode-line
...
#+begin_src emacs-lisp :noweb-ref org-headers
...
I like having line numbers and indicators for lines past the EOF.
#+begin_src emacs-lisp
...
*** Easily find a string in a buffer's file name
...
** On startup
   ...
   :end:
*** Make Emacs go fullscreen
This is for visual consistency. I personally like it this way.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp :noweb-ref on-logout
