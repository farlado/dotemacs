...
First, we load ~org-tempo~, the extension that allows the old way of doing things. Then, we add shortcuts for the individual blocks of code. Finally, we can add shortcuts for other items that aren't blocks.
#+begin_src emacs-lisp
  (require 'org-tempo)
