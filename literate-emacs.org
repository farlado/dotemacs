...
** Dependencies
   ...
   <p align="center">
     <b>
       THIS CONFIGURATION IS MEANT FOR EMACS 27 AND LATER. IT WILL MOST LIKELY
       <i>NOT</i> LOAD PROPERLY ON EMACS 26 OR EARLIER. THE BRANCH FOR EMACS 26
       AND EARLIER CAN BE FOUND
       <a href="https://github.com/farlado/dotemacs/tree/emacs26-end">HERE</a>.
     </b>
   </p>
   #+end_export
...
** Theme
   ...
   Leuven is my theme of choice. Leuven is king. Long live Leuven. This theme is
   /brilliant/. It took months to find a theme that could hold a candle to Leuven,
   and even then it failed to stick with me as well as Leuven has. Once you can
   appreciate a light theme, it becomes increasingly more difficult to drop it.
...
** Mode line
   ...
   #+begin_src emacs-lisp :noweb yes :tangle (locate-user-emacs-file "init.el")
     (use-package mood-line
       ...
       (mood-line-mode 1)
       <<mode-line-init>>
       :custom-face <<mode-line-faces>>
       <<mode-line-vars>>)
   #+end_src

*** Change a hard-coded face by force

    This frustrates me a lot, but at least there's a way to fix it.
...
*** Use some less ridiculous colors for the mode line

    As much as I love Leuven, it definitely isn't the most friendly towards this
    mode line. In order to fix this, we set a few faces accordingly.

    #+begin_src emacs-lisp :noweb-ref mode-line-faces
      (mood-line-status-info ((t (:inherit mode-line) (:background nil))))
      (mood-line-unimportant ((t (:inherit mode-line) (:background nil))))
      (mood-line-status-neutral ((t (:inherit mode-line) (:background nil))))
    #+end_src
...
*** Show clock and battery level on mode line
    ...
    one less package to configure lol.
    ...
    The clock should be in 24-hour time, and the date should also be shown.

    #+begin_src emacs-lisp :noweb-ref mode-line-vars
